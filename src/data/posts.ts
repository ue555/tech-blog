...
### 2. 基調講演: Introducing the TabPanel Feature: Motivation and Implementation

**rbtnn** さんによる基調講演です。**rbtnn** さんが作成されたTabPanel(Vimのv9.1.1391に導入された機能)の作成過程やVimどうして、このような機能を作成したいと思ったのかということを紹介されていました。
...
`,
  ...
  {
    id: 5,
    title: "2025年年末、新宿もくもく会に参加しました",
    excerpt:
      "新宿もくもく会初参加です！！",
    content: `
### はじめに

2025年年末、新宿もくもく会に参加しました。
普段、お付き合いのある顔馴染みの方と、お互いにおのおの個人開発を持ち合って、年が明けるまで黙々と作業をしました。

私は、以前個人開発をした家計簿のアプリをAWSにデプロイする環境を今回のもくもく会で作成しました。
...
\`\`\`
  ...
  - VPC (10.0.0.0/16) を2つのAZに分散
    - Public Subnets: ALB、NAT Gateway配置
    - Private App Subnets: ECS Tasks、App Runner VPC Connector配置
    - Private DB Subnets: RDS MySQL配置

  フロントエンド

  1. ユーザーが kakeibo-apps.com にアクセス
  2. Route53 → CloudFront (CDN) → App Runner (React/Vite)
  3. App RunnerからVPC Connectorを経由してバックエンドAPIにアクセス

  バックエンド

  1. ユーザーまたはフロントエンドがドメインにアクセス
  2. Route53 → ALB (Public) → ECS Fargate (Node.js) → RDS MySQL
  3. ECSタスクは0.25vCPU/512MBで1〜3台でAuto Scaling

  データベース

  - RDS MySQL 8.0 (db.t3.micro、20GB〜100GB自動拡張)
  - Private DB Subnetsに配置、ECSとCodeBuildからのみアクセス可能

  CI/CD

  - GitHub → Webhook → CodeBuild → ECR → デプロイ
    - バックエンド: ECRにプッシュ後、ECSサービスを自動更新
    - フロントエンド: ECRにプッシュ後、App Runnerが自動デプロイ

  セキュリティ

  - Security Groupsでトラフィック制御 (ALB→ECS→RDS)
  - Secrets ManagerでDB認証情報を管理
  - ACM証明書でHTTPS通信 (CloudFront用とALB用)

  モニタリング

  - CloudWatch Logsで全コンポーネントのログ集約
  - S3でCodeBuildのキャッシュ保存
\`\`\`
---------

### 2. terraformのコードをバイブコーディングで作成しました!!

業務で利用できるようになるために、terraformのコードをバイブコーディングで作成しました。(terraformの外観を掴むために)
実際、作業をAIに投げてみると、フロントエンドとバックエンドとのAPIの疎通だったり、ドメインの設定だったりとハマりどころがいっぱいでした。
3時間ぐらいで終わるかと思ったのですが、結局10時間ぐらいかかってます。

私のAWSのサービスで理解していない箇所等を実際、私が作成したアプリのデプロイAWSのデプロイ環境を作成して、理解が深まったように思います。

---------

### 4. 今後について。

実際に、家計簿アプリを利用して、問題点を洗い出したり、AWSの構成を変更して安く運用できるようにしたいです。
また、アプリ側の機能も増やしていきたいと思ってます。

---------

### 5. 参加されたメンバーの方はどうだったか！！

皆さん、Vimのプラグインを作成される方や、ゲームを作成される方、Nostrについて、作業される方、各々集中されて作業されているようでした。
その中で、Nostrの発火さんの投稿を拾ってきて、タイピングするゲームが、メンバーの中で受けていて面白かったです。

新宿もくもく会ですが、実質、目黒で開催したので、実質めぐろvimでした。(オチです)

---------

### まとめ

14時間という長丁場で、作業しました。目的の作業をやり切った感と疲れで大満足です。
このような体験は、貴重です。また、来年開催されたら参加したいです。

皆様、良いお年を!! Good Luck!!

追伸、年末公共交通機関って意外と動いてないのですね。。。。(通常、運行でした)
`,
    date: "2026-01-02",
    tags: ["新宿もくもく会", "vim-jp"],
    imageUrl:
